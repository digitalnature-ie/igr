...
Irish grid references are a traditional way of recording locations on the Irish Grid, and were particularly relevant when dealing with paper-based maps. An Irish grid reference consists of a letter, optionally followed by an easting and northing. A letter alone (e.g. "N") refers to a particular 100 km square on the Irish Grid. An Irish grid reference with a 1-digit easting and northing (e.g. "N85") refers to a 10 km square. A 5-digit easting and northing (e.g. "N 12345 67890") refers to a 1 m square.

There are many R packages facilitating [spatial data analysis and visualisation](https://cran.r-project.org/web/views/Spatial.html). The igr package is designed to simplify using datasets containing Irish grid references in R. It translates Irish grid references into Irish Grid coordinates or sf objects that other R packages can work with. The igr package supports all resolutions of Irish grid reference, datasets with mixes of resolutions, and Irish grid references with or without whitespace between letter, easting and northing.
...
These sample Irish grid references have a mix of resolutions. The resolutions in metres can be returned by supplying a column name for the resolution data:
...
The Irish grid references can also be converted to polygon geometries. Each polygon spans the full extent of a grid reference, so if grid references with a mix of resolutions are supplied, the polygons will have a mix of sizes.
...
The Irish grid references can be limited to a particular resolution by specifying a number of digits between 0 and 5:

```{r example-ig-3}
ig_to_igr(p, digits = 1) # 10 km

ig_to_igr(p, digits = 5) # 1 m
```
